// ═══════════════════════════════════════════════════════════════════
// CREATOMATE BODY BUILDER — v5.8.1: WHOLE-PHOTO + BLURRED FILL + SLOW PUSH-IN
// Same as v5.8 (no clip ever repeats, slow centered push-in, photos show the
// whole image over a blurred fill from frame one, videos unchanged), but the
// FALLBACK path now degrades gracefully on a short song: visual elements are
// re-clamped to the composition total and the trailing reversed fade-out is
// applied there too (parity with the main capped path).
//
// (1) IMAGES = TWO LAYERED ELEMENTS.
//     a) BLURRED BACKGROUND (lower track): same source, fills the frame,
//        blur_radius 50, slight dark overlay. Holds still.
//     b) FOREGROUND WHOLE PHOTO (higher track, on top): fit:'contain' so the
//        ENTIRE image is visible, with the slow centered push-in (100% ->
//        (100+KB_ZOOM)% over the shot).
//
// (2) TRACK LAYERING: a single running counter assigns one track per element
//     in emission order. A photo consumes TWO tracks (bg then fg); a video
//     consumes one. Tracks strictly ascend with time. AUDIO sits above every
//     visual track.
//
// (3) STRICTLY NO REPEATED CLIPS. Every clip index appears AT MOST ONCE. A
//     photo's bg+fg are the SAME clip at the SAME time — one appearance.
// ═══════════════════════════════════════════════════════════════════
...
const BLUR_RADIUS = 50;    // blurred backdrop fill behind the whole photo
...
// Emit a PHOTO as two layered elements (blurred fill + whole-photo push-in) onto `arr`.
// trkRef.v supplies/advances the running track counter (bg, then fg).
...
function pushPhoto(arr, url, time, dur, trkRef, withFade) {
  // 1) blurred background — fills the frame, holds still, just fades in
  const bg = { type: 'image', source: url, track: trkRef.v++, time: r2(time), duration: r2(dur),
    blur_radius: BLUR_RADIUS, color_overlay: 'rgba(0,0,0,0.18)', clip: true,
    width: '100%', height: '100%', x: '50%', y: '50%' };
  if (withFade) bg.animations = [fadeIn];
  arr.push(bg);
  // 2) foreground whole photo — fit:'contain', slow centered push-in, on top
...
function footagePack() {
  ...
  videoClips.forEach((vc) => {
    ...
    if (isImg) {
      pushPhoto(els, vc.url, cur, d, trkRef, !first);   // v5.8: whole photo + blurred fill
    } else {
...
if (!Array.isArray(seq) || seq.length === 0) {
  ...
  return [{ json: { creatomate_body: JSON.stringify(body), job_id: jobId, totalDuration: body.source.duration, timeline_mode: 'footage_fallback_v5.8', clip_count: body.source.elements.filter(e => e.type !== 'audio').length } }];
}
...
if (!shows.length) {
  const body = footagePack();
  return [{ json: { creatomate_body: JSON.stringify(body), job_id: jobId, totalDuration: body.source.duration, timeline_mode: 'footage_fallback_v5.8' } }];
}
...
if (!endedEarly) {
  ...
  for (const idx of rest) {
    ...
    const trkRef = { v: trk };
    if (isImage) {
      pushPhoto(elements, ci.url, start, dur, trkRef, true);   // v5.8: whole photo + blurred fill
    } else {
...
return [{ json: { creatomate_body: JSON.stringify(body), job_id: jobId, totalDuration: r2(totalDuration), song_len: songLen,
  ...
  distinct_clips_shown: distinctClips, timeline_mode: 'footage_capped_v5.8' } }];