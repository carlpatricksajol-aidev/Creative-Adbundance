...
// categories that carry on TYPE/concept (no physical product needed) — safe for review sites, services, finance
const TYPOGRAPHIC = /headline|question|statistic|comparison|us vs|split screen|strong copy|listicle|social proof|pr callout|infographic|offer|big logo|business|professional|\bclaim|\bnote|twitter|\btext\b|\bquote|review|testimonial|framing/i;
async function selectTemplates(brain, count, hasProduct) {
  let idx = (await loadTemplates()).filter(r => !SEASONAL.test(r.category || ''));
  if (!hasProduct) idx = idx.filter(r => TYPOGRAPHIC.test(r.category || '')); // no product → typographic templates only (no product-hero layouts)
