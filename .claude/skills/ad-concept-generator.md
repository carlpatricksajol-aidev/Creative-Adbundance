...
> **Provenance and status.** Authored by Ricardo Mestae. Added to this repo on 2026-08-19.
>
> **This file is the SKILL.md only.** The skill references five files under `references/` and one
> under `scripts/` that are NOT in this repo yet and are required to run it as designed:
> `references/craft-rules.md`, `references/creative-strategist.md`, `references/libraries.md`,
> `references/image-prompts.md`, `references/config-example.json`, `scripts/build_deck.js`.
> Get those from Ricardo and drop them in `.claude/skills/ad-concept-generator/` before running.
> Without them the pipeline below still describes the method, but the strategist scorecard, the
> observation-prompt bank, and the deck builder are missing.
>
> **Client names generalized.** This repository is public. The original names four accounts and one
> account's internal batch-survival outcome. Those are replaced with role descriptors here; every
> craft rule and every number is unchanged. The unredacted original is kept outside the repo at
> `_vault-local/skills/ad-concept-generator-UNREDACTED.md`.
...
prints without passing the strategist. v4 adds batch composition targets learned from a survival diff
on a telehealth account's Batch 3: only 3 of 16 generated concepts survived client finalization, and the
replacements were differentiator-built, stat-led, second-character, trend-templated, tonally varied, and
awareness-tagged. Individually-good concepts are not enough, the BATCH has a composition.

## The core shift (why this exists)

Real client feedback across four accounts (a telehealth brand, two social-growth tools, and a parenting
app) converged on the same diagnosis: earlier versions of this skill wrote **ads that communicated
information**. Good creative directors write **concepts a CD checks off and moves into refinement**. The
gap is not more craft rules, it's a different starting point:

- **Bad flow:** *insight, then ad.* Take a benefit, wrap a UGC vehicle around it, describe why it works.
- **Good flow:** *human observation, then creative leap, then vehicle, then the brand slots in.* Start
  from a specific behavior/situation/thought someone would recognize. Find an unexpected but immediately
  understandable device for expressing it. The brand fits into the vehicle; the vehicle isn't decoration.

Concrete examples of the difference (all from the telehealth account's feedback):

- Refill friction. **Bad:** "Woman realizes she's running out, gets appointment friction, discovers the
  brand." **Good:** *"Vacation in seven days + out of birth control"*, a countdown POV with packing
  b-roll, delivery, payoff.
- Affordability. **Bad:** "Compare our pricing with traditional care." **Good:** *"My Bank Account Is
  Confused"*, the bank account becomes the storyteller; receipts are the visual language.
- Personalized HRT. **Bad:** "Why personalization matters." **Good:** *"HRT Dating Profile"*, the format
  IS the ad; the message rides inside.
...
- **The creative leap.** Don't turn the observation into a straightforward dramatization ("woman
  realizes she's out of BC, discovers the brand"). Find an unexpected but immediately understandable
  device, the vehicle IS the ad. "Vacation countdown," "bank account is confused," "dating profile,"
  "toxic trait confession," "did you know?" reveal, "group chat blowing up," "phone side quest." See the
...
- **5-dimension diversity spread?** Angle x Human situation x Format x Visual device x Funnel job.
...
- **Batch composition checks (v4, from the survival diff):** differentiator share when a
...
- **Title.** `NNN_Title`, brand accent color. **Must name or strongly imply the creative vehicle.**
  "HRT Dating Profile" yes. "Bank Account Is Confused" yes. "Vacation Countdown" yes.
  "I Became My Own Doctor" no (it's a line, not a vehicle).
...
targets came from exactly this exercise on the telehealth account's Batch 3 (3 of 16 survived; the
replacements were differentiator-built, stat-led, second-character, trend-templated, funnier, and
awareness-tagged).
