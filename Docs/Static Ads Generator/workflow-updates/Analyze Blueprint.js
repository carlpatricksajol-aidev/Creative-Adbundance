// Analyze Blueprint - forensic content analyzer for the static ads pipeline.
//
// WHAT IT DOES: for each slot, reads the BLUEPRINT image the ad is modeled on
// (item.template_url = the uploaded reference, the client's winning ad, a promo-variant
// anchor, or a picked/library template) and produces a forensic text spec
// (blueprint_analysis). Build KIE AI Prompt and Generate Ad Copy then use that spec to
// reproduce the source ad's concept, layout, and COPY STRUCTURE (the copy node otherwise
// never sees the image).
//
// n8n setup:
//   - Node type: Code, mode = "Run Once for All Items"
//   - Name it EXACTLY "Analyze Blueprint" (Generate Ad Copy looks it up by that name).
//   - Wire it BETWEEN Pick Templates and Build KIE AI Prompt:
//       Pick Templates -> Analyze Blueprint -> Build KIE AI Prompt
//   - Paste your rotated OpenRouter key below (same key as Generate Ad Copy / QA Pass).
//
// Cost note: one vision call per UNIQUE blueprint URL (deduped). Manual-pick runs with N
// distinct templates = N calls. A miss never breaks the run (returns empty analysis).

const OPENROUTER_KEY = '<OPENROUTER_API_KEY>';
const MODEL = 'anthropic/claude-sonnet-4';  // vision-capable; strong for forensic detail

const ANALYZER_PROMPT = `You are analyzing a single static ad image. Your only job is to produce a complete, exhaustive, replicable record of the creative — a forensic transcript of every visible element, every zone of the layout, every word of on-image text, and every design detail, captured exactly as it appears. Capture everything an attentive observer could note; this description may afterward be used for many different purposes, so leave nothing out and assume nothing about which details matter. You are not a critic. You are not a strategist. You are a transcriber. Describe only what is observable; never write impressions, never praise, never guess.
...
for (const entry of items) {
  const j = entry.json;
  const url = j.template_url ? String(j.template_url) : '';
  let analysis = '';
  if (url) {
    if (Object.prototype.hasOwnProperty.call(cache, url)) {
      ...
      try {
        ...
        analysis = (resp && resp.choices && resp.choices[0] && resp.choices[0].message && resp.choices[0].message.content) || '';
      } catch (e) {
        analysis = '';  // never hard-fail a run on a blueprint-analysis miss
      }
      cache[url] = analysis;
    }
  }
  out.push({ json: Object.assign({}, j, { blueprint_analysis: analysis }) });
}
