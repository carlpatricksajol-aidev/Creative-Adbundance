// Stripe webhook → tops up credits when a Checkout payment completes.
// Protected by a shared secret in the URL (?key=…). Instead of raw-body signature
// verification, we re-fetch the session from Stripe with the secret key — that's
...
module.exports = async (req, res) => {
  ...
  if (!sessionId) { res.status(400).json({ error: 'no session id' }); return; }
  ...
  // --- record idempotently (provider_ref = Stripe session id) → credit only if NEW ---
  let inserted = [];
  try {
    const r = await fetch(`${SUPABASE_URL}/rest/v1/paid_orders?on_conflict=provider_ref`, {
      ...
      body: JSON.stringify({ provider_ref: sessionId, order_number: sessionId, email, amount, plan, status: 'paid' })
    });
